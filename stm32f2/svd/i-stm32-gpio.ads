...
package Interfaces.STM32.GPIO is
   ...
   type GPIO_Peripheral is record
      --  GPIO port mode register
      MODER   : aliased MODER_Register with Volatile_Full_Access;
      --  GPIO port output type register
      OTYPER  : aliased OTYPER_Register with Volatile_Full_Access;
      --  GPIO port output speed register
      OSPEEDR : aliased OSPEEDR_Register with Volatile_Full_Access;
      --  GPIO port pull-up/pull-down register
      PUPDR   : aliased PUPDR_Register with Volatile_Full_Access;
      --  GPIO port input data register
      IDR     : aliased IDR_Register with Volatile_Full_Access;
      --  GPIO port output data register
      ODR     : aliased ODR_Register with Volatile_Full_Access;
      --  GPIO port bit set/reset register
      BSRR    : aliased BSRR_Register with Volatile_Full_Access;
      --  GPIO port configuration lock register
      LCKR    : aliased LCKR_Register with Volatile_Full_Access;
      --  GPIO alternate function low register
      AFRL    : aliased AFRL_Register with Volatile_Full_Access;
      --  GPIO alternate function high register
      AFRH    : aliased AFRH_Register with Volatile_Full_Access;
   end record
     with Volatile;
